...
public class MapUtils {
	...
	public static void createJSONFile(FeatureCollection fc) {
    	try {
    		...
    		String fileName = "readings-" + App.day + "-" + App.month + "-" + App.year + ".geojson";
	...
	public static void createTextFile(String flightpath) {
		try {
			...
			String fileName = "flightpath-" + App.day + "-" + App.month + "-" + App.year + ".txt";
    		FileWriter file = new FileWriter(fileName);
	...
	// gets initial list of markers from list of sensors to be read for the current day
	...
	public static String getSensorColour(Sensor sensor) {
    	...
    	// only use reading if battery is not low - never come across a NaN value
